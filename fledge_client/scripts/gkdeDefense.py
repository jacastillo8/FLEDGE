...
def main():
    mnist_model_template = get_model_template('MNIST')
    mnist_model_dir = '../models/{}/pytorch_models'.format('MNIST')
    # Collect global model
    mnist_global_model = get_model(mnist_model_template, '{}/G5.pt'.format(mnist_model_dir))
    mnist_local_models = []
    # Collect available local models
    for m in os.listdir(mnist_model_dir):
        # Provided global model has been trained for 5 rounds (G5)
        if m != 'G5.pt':
            model = get_model(mnist_model_template, '{}/{}'.format(mnist_model_dir, m))
            mnist_local_models.append(model)
    scores = get_scores(mnist_global_model, mnist_local_models, 'MNIST')
    ids = filter_scores(scores)
    # Selected MNIST Models (see below) can be used for aggregation 
    filtered_models = get_sub_list(mnist_local_models, ids)
    print('############ MNIST Learning Task - GKDE Defense ############')
    print('######### Benign_Length: {} - Malicious_Length: {} #########'.format(len(ids), len(scores) - len(ids)))

    cifar_model_template = get_model_template('CIFAR10')
    cifar_model_dir = '../models/{}/pytorch_models'.format('CIFAR10')
    # Collect global model
    cifar_global_model = get_model(cifar_model_template, '{}/G50.pt'.format(cifar_model_dir))
    cifar_local_models = []
    # Collect available local models
    for m in os.listdir(cifar_model_dir):
        # Provided global model has been trained for 50 rounds (G50)
        if m != 'G50.pt':
            model = get_model(cifar_model_template, '{}/{}'.format(cifar_model_dir, m))
            cifar_local_models.append(model)
    scores = get_scores(cifar_global_model, cifar_local_models, 'CIFAR10')
    ids = filter_scores(scores)
    # Selected CIFAR10 Models (see below) can be used for aggregation 
    filtered_models = get_sub_list(mnist_local_models, ids)
    print('########### CIFAR10 Learning Task - GKDE Defense ###########')
    print('######### Benign_Length: {} - Malicious_Length: {} #########'.format(len(ids), len(scores) - len(ids)))
